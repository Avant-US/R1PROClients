...
class TrajectoryStitcher:
    ...
    def _find_nearest_action_joint_state(self, current_traj: Trajectory, new_traj: Trajectory, action_frequency: float = 15.0) -> Tuple[int, int]:
        ...
        距离计算基于所有非 None 的关节/底盘数据，包括位置、速度和加速度。
        速度和加速度根据位置序列和频率计算得出。
        ...
        def extract_field_array(action: RobotAction, field_name: str) -> np.ndarray | None:
            field = getattr(action, field_name, None)
            if field is None:
                return None
            if hasattr(field, 'position'):
                return np.array(field.position, dtype=np.float64)
            if field_name == 'chassis' and isinstance(field, TwistStamped):
                return np.array([
        ...
        for action in list(current_traj.actions) + list(new_traj.actions):
            for field_name in joint_state_fields:
                field_array = extract_field_array(action, field_name)
        ...
        def extract_joint_state_vector(action: RobotAction, action_idx: int, 
            ...
            for field_name in joint_state_fields:
                ...
                position_sequence = []
                for traj_action in trajectory_actions:
                    pos_array = extract_field_array(traj_action, field_name)
